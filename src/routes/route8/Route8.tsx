...
function CameraController() {
  ...
  const goToView = (viewNumber: number) => {
    if (viewNumber >= 1 && viewNumber <= 3) {
      ...
      api.start({
        position: targetView.position as [number, number, number],
        fov: targetView.fov,
      });
      console.log(`React Spring Transitioning to View ${viewNumber}: ${targetView.name}`);
    }
  };
...
export default function Route8() {
  ...
  const goToView = (viewNumber: number) => {
    if (viewNumber >= 1 && viewNumber <= 3) {
      ...
      // Call the React Spring function
